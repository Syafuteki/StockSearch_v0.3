...
def normalize_bar_row(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if code is None or trade_date is None:
        return None
    open_price = to_float(pick_first(row, ["Open", "OpenPrice", "open", "opening_price"]))
    high_price = to_float(pick_first(row, ["High", "high", "HighPrice"]))
    low_price = to_float(pick_first(row, ["Low", "low", "LowPrice"]))
    close_price = to_float(pick_first(row, ["Close", "close", "ClosePrice"]))
    adj_close = to_float(
        pick_first(
            row,
            [
    ...
    return {
        ...
        "close": close_price,
        "adj_close": adj_close,
        "volume": to_int(pick_first(row, ["Volume", "volume", "TradingVolume"])),
        "market_cap": to_float(pick_first(row, ["MarketCapitalization", "market_cap", "MarketCap"])),
        "raw_json": row,
    }

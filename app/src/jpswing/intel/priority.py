...
def calculate_priority(item: PriorityInput) -> float:
    ...
    score = (
        ...
        + (0.20 if item.has_new_edinet else 0.0)
        + (max(0.0, min(1.0, item.theme_strength)) * 0.15)
        + (max(-1.0, min(1.0, item.theme_strength_delta)) * 0.05)
        + (0.05 if item.has_high_signal_tag else 0.0)
    )
